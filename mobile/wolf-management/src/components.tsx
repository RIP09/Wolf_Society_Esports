    ...
    variant === "danger"
      ...
      : variant === "green"
        ...
        : variant === "ghost"
          ? COLORS.card
          : COLORS.yellow;
  const fg = variant === "danger" || variant === "green" ? "#fff" : COLORS.ink;
  return (
    <Pressable
      onPress={onPress}
...
export function Stat({ label, value, sub, color = COLORS.yellow }: { label: string; value: string; sub?: string; color?: string }) {
  return (
...
export function Header({ eyebrow, title }: { eyebrow: string; title: string }) {
  return (
    <View style={{ gap: 4 }}>
      <Text style={[s.eyebrow, { color: COLORS.orange }]}>{eyebrow}</Text>
